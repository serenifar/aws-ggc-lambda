            ...
            while have_tried < max_attempts:
                try:
                    ...
                    if total_wait_time + duration > expiration_duration:
                        raise RetryTimeoutException(task_to_try.__name__, max_attempts, total_wait_time, enable_jitter,
                                                    retry_errors)

                    runtime_logger.warn('Retrying [{0}], going to sleep for {1} seconds'.format(task_to_try.__name__, duration))
                    time.sleep(duration)
